...
## 애플리케이션 정보
`❗ 만약 웹 애플리케이션을 Visual Basic이나 다른 언어로 작성할 경우 이 내용을 참고해주세요.`
...
#### 조회 버튼
버튼을 클릭했을 때 처리하는 기능은 [조회 버튼을 클릭했을 때](#조회-버튼을-클릭했을-때)를 확인해주세요
#### 분석 버튼
버튼을 클릭했을 때 처리하는 기능은 [분석 버튼을 클릭했을 때](#분석-버튼을-클릭했을-때)를 확인해주세요
#### 학습 버튼
버튼을 클릭했을 때 처리하는 기능은 [학습 버튼을 클릭했을 때](#학습-버튼을-클릭했을-때)를 확인해주세요
#### 저장 버튼
저장 버튼은 모달 안에 있습니다.
버튼을 클릭했을 때 처리하는 기능은 [저장 버튼을 클릭했을 때](#저장-버튼을-클릭했을-때)를 확인해주세요
#### 취소 버튼
취소 버튼은 모달 안에 있습니다.
버튼을 클릭했을 때 처리하는 기능은 [취소 버튼을 클릭했을 때](#취소-버튼을-클릭했을-때)를 확인해주세요
#### 삭제 버튼
버튼을 클릭했을 때 처리하는 기능은 [삭제 버튼을 클릭했을 때](#삭제-버튼을-클릭했을-때)를 확인해주세요
...
#### 분석 버튼을 클릭했을 때
1. [분석 검사 조회 API 요청](#분석-검사-조회)
2. [분석 조회 API 요청](#분석-조회)
3. 현재 인공지능 정확도 필드에 분석 조회 API 응답 데이터의 0번째 인덱스 `accuracy` 값 할당
3. 종료일 필드의 값을 오늘로 변경
4. 분석 여부 값을 true로 변경
#### 학습 버튼을 클릭했을 때
...
#### 분석 여부 값이 true로 변경되었을 때
1. [메인 데이터 그리드 조회 API 요청](#메인-화면-데이터-그리드-조회)
2. 메인 데이터 그리드에 API 응답 데이터 할당
3. 분석 여부 값을 false로 변경

#### 삭제 버튼을 클릭했을 때
1. 확인 메시지 출력`("데이터를 삭제하시겠습니까?")`
2. [메인 화면 데이터 그리드 삭제 API 요청](#메인-화면-데이터-그리드-삭제)
3. [재 조회](#조회-버튼을-클릭했을-때) 이벤트 실행
